...
def softmax_loss(x, y):
    ...
    num_train = x.shape[0]
    stabilised_x = x - np.max(x, axis=1).reshape(-1,1)
    # correct_class_score = x[np.arange(num_train),y]
    scores = np.exp(stabilised_x)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = mu, var, std, eps, gamma, beta, x_hat, axis
...
def batchnorm_backward(dout, cache):
    ...
    mu, var, std, eps, gamma, beta, x_hat, axis = cache
    ...
    dvar = (-0.5/(var + eps))*((dx_hat * x_hat).sum(axis=0))
    dx3 = (2/num_train)*std*dvar*x_hat
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx1 = dout * gamma/(m*std)
    dx2 = dx1.sum(axis=0)
    dx3 = x_hat*((dx1*x_hat).sum(axis=0))
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    H1 = int(1 + (H + 2 * pad_size - HH)//stride)
    W1 = int(1 + (W + 2 * pad_size - WW)//stride)

    x_pad = np.pad(x, pad_size, 'constant')[pad_size:pad_size+len(x)][:,1:-1,:,:]
      ...
      for f in range(F):
        ...
        for i in range(0,H1):
          for j in range(0,W1):
            col_start = i*stride
            row_start = j*stride
            out[n,f,i,j] = np.sum((img[:,col_start:col_start+HH,row_start:row_start+WW] * filter_)) + bias_
      ...
      for f in range(F):
        ...
        for i in range(0,H1):
          for j in range(0,W1):
            ...
            dw[f] += img[:,col_start:col_start+HH,row_start:row_start+WW] * dout[n,f,i,j]
            dx_pad[n,:,col_start:col_start+HH,row_start:row_start+WW] += filter_ * dout[n,f,i,j]
      ...
      for c in range(C):
        ...
        for i in range(0,H1):
          for j in range(0,W1):
            col_start = i*stride
            row_start = j*stride
            out[n,c,i,j] = np.max(img[col_start:col_start+pool_height, row_start:row_start+pool_width])
      ...
      for c in range(C):
        ...
        for i in range(0,H1):
          for j in range(0,W1):
            col_start = i*stride
            row_start = j*stride
            ind = np.argmax(img[col_start:col_start+pool_height,row_start:row_start+pool_width])
            
            row, col = np.unravel_index(ind, (pool_height, pool_width))

            dx[n,c,col_start:col_start+pool_height,row_start:row_start+pool_width][row,col] = dout[n,c,i,j] 
